...
/* The class implements mapping for each field in the request and therefore the excessive number of methods is OK. */
public class RingRequestAdapter implements Handler<RoutingContext> {
  
  private static String stringJoiner(Iterable<String> v) {
    return String.join(",", v);
  }
  
  private static IPersistentVector toVector(Collection<?> v) {
  ...
  private IPersistentMap getImmutableRequest(RoutingContext ctx) {
    Collection<Object> values = new ArrayList<>(14 << 1);
  ...
  private RingRequestAdapter addServerPort(RoutingContext ctx, Collection<Object> values) {
    return addNameValuePair(SERVER_PORT.keyword(), SERVER_PORT.get(ctx), values);
  }
  
  private RingRequestAdapter addServerName(RoutingContext ctx, Collection<Object> values) {
    return addNameValuePair(SERVER_NAME.keyword(), SERVER_NAME.get(ctx), values);
  }
  
  private RingRequestAdapter addRemoteAddress(RoutingContext ctx, Collection<Object> values) {
    return addNameValuePair(REMOTE_ADDRESS.keyword(), REMOTE_ADDRESS.get(ctx), values);
  }
  
  private RingRequestAdapter addUri(RoutingContext ctx, Collection<Object> values) {
    return addNameValuePair(URI.keyword(), URI.get(ctx), values);
  }
  
  private RingRequestAdapter addScheme(RoutingContext ctx, Collection<Object> values) {
    return addNameValuePair(SCHEME.keyword(), SCHEME.get(ctx), values);
  }
  
  private RingRequestAdapter addMethod(RoutingContext ctx, Collection<Object> values) {
    return addNameValuePair(REQUEST_METHOD.keyword(), REQUEST_METHOD.get(ctx), values);
  }
  
  private RingRequestAdapter addProtocol(RoutingContext ctx, Collection<Object> values) {
    return addNameValuePair(PROTOCOL.keyword(), PROTOCOL.get(ctx), values);
  }
  
  private RingRequestAdapter addSslClientCert(RoutingContext ctx, Collection<Object> values) {
    return addNameValuePair(CLIENT_CERT.keyword(), CLIENT_CERT.get(ctx), values);
  }
  
  @SuppressWarnings("UnusedReturnValue")
  private RingRequestAdapter addBody(RoutingContext ctx, Collection<Object> values) {
    return addNameValuePair(BODY.keyword(), BODY.get(ctx), values);
  }
  
  private RingRequestAdapter addHeaders(RoutingContext ctx, Collection<Object> values) {
  ...
  private RingRequestAdapter addQueryString(RoutingContext ctx, Collection<Object> values) {
    return addNameValuePair(QUERY_STRING.keyword(), QUERY_STRING.get(ctx), values);
  }
  
  private RingRequestAdapter addQueryParams(RoutingContext ctx, Collection<Object> values) {
    return addNameValuePair(QUERY_PARAMS.keyword(), toPersistentMap((MultiMap) QUERY_PARAMS.get(ctx)), values);
  }
  
  private RingRequestAdapter addPathParams(RoutingContext ctx, Collection<Object> values) {
  ...
  private RingRequestAdapter addFormParams(RoutingContext ctx, Collection<Object> values) {
  ...
  private RingRequestAdapter addNameValuePair(Keyword name, Object value, Collection<Object> values) {
    if (value != null) {
      values.add(name);
      values.add(value);
    }
